...
with st.sidebar:
    ...
    if st.session_state.step_data:
        df_s = st.session_state.ohlcv_final if st.session_state.ohlcv_final is not None else (st.session_state.ohlcv_raw if st.session_state.ohlcv_raw is not None else pd.DataFrame())
        if not df_s.empty:
...
# ══════════════════════════════════════════════════════════
    ...
    with cr2:
        ...
        if not ptk.empty:
            ...
            fig_t.update_layout(**PLOTLY_DARK,height=240,yaxis_title="Avg Ret (%)",showlegend=False,title=dict(text=""))
            fig_t.update_xaxes(tickangle=-35)
